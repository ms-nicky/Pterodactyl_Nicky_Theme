...
installTheme(){
    ...
    tar -cvf Pterodactyl_Nicky_Themebackup.tar.gz pterodactyl > /dev/null 2>&1
    echo -e "${GREEN}Installing theme... ${RESET}"
    cd /var/www/pterodactyl > /dev/null 2>&1
    echo -e "${GREEN}Removing old theme if exist${RESET}"
    rm -r Pterodactyl_Nicky_Theme > /dev/null 2>&1
    echo -e "${GREEN}Download the Theme${RESET}"
    git clone https://github.com/ms-nicky/Pterodactyl_Nicky_Theme.git > /dev/null 2>&1
    cd Pterodactyl_Nicky_Theme > /dev/null 2>&1
    echo -e "${GREEN}Removing old theme resources if exist${RESET}"
    rm /var/www/pterodactyl/resources/scripts/Pterodactyl_Nicky_Theme.css > /dev/null 2>&1
    ...
    mv Pterodactyl_Nicky_Theme.css /var/www/pterodactyl/resources/scripts/Pterodactyl_Nicky_Theme.css > /dev/null 2>&1
    cd /var/www/pterodactyl > /dev/null 2>&1
...
repair(){
    bash <(curl https://raw.githubusercontent.comms-nicky/Pterodactyl_Nicky_Theme/main/repair.sh)
}

restoreBackUp(){
    echo "Restoring backup..."
    cd /var/www/ > /dev/null 2>&1
    tar -xvf Pterodactyl_Nicky_Themebackup.tar.gz > /dev/null 2>&1
    rm Pterodactyl_Nicky_Themebackup.tar.gz > /dev/null 2>&1

    cd /var/www/pterodactyl > /dev/null 2>&1
    yarn build:production > /dev/null 2>&1
    sudo php artisan optimize:clear > /dev/null 2>&1
}
echo "Copyright (c) 2025 Angelillo15 and ms-nicky"
